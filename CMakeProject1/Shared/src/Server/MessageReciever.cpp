                                      ...
                                      std::shared_ptr<std::string> napis)
{
    ...
    boost::asio::async_read(*socket, boost::asio::buffer(*body_buffer),
                            [header_buffer, body_buffer, this, socket, napis](
                                        ...
                                        std::shared_ptr<std::string> napis)
{
    ...
    boost::asio::async_read(*socket, boost::asio::buffer(*header_buffer),
                            [header_buffer, header_size, this, socket, napis](
                            const boost::system::error_code& err, std::size_t bytes_transferred)
                            {
                                if (err)
                                {
                                ...
                                {
                                    std::memcpy(&body_length, header_buffer->data() + sizeof(uint32_t),
                                                sizeof(uint32_t));
                                ...
                                if (body_length != 0) {start_read_body(header_buffer, body_length, socket, napis); return;}
